...
->group(function () {
    ...
    Route::post('/account', 'BaseUser@account');//权证账户信息
    Route::post('/usdt/recharge', 'BaseUser@usdt_recharge');//usdt充值记录
    Route::post('/account/log', 'BaseUser@account_log');//权证用户收支记录
    Route::post('/qz/user_list', 'BaseUser@user_list');//权证用户列表
    Route::post('/users', 'UserController@store');

    //权证后台产品发布
    Route::post('/create/product', 'QzProduct@create');

    //权证产品列表
    Route::get('/product/lists', 'QzProduct@lists');

    //查看单个权证产品
    Route::post('/product/data', 'QzProduct@data');

    //权证产品撤回
    Route::post('/product/withdraw', 'QzProduct@withdraw');


    //权证订单列表
    Route::post('/order/lists', 'OrderController@lists');

    //权证产品草稿箱列表
    Route::post('/product/draft', 'QzProduct@draft');

     //权证产品草稿箱发布
    Route::post('/new/product', 'QzProduct@news');

    //权证草稿箱删除
    Route::post('/product/delete', 'QzProduct@delete');

    //提现申请审核列表
    Route::post('/forward/lists', 'WithdrawalsApply@lists');

    //返回权证币种
    Route::get('/get/tokens', 'TokenController@return_token');

    //权证订单列表导出
    Route::get('/order/export', 'OrderController@export_order');

    //提现审核操作(通过/拒绝)
    Route::post('/examine/operation', 'WithdrawalsApply@examine');

    //设置提现手续费
    Route::post('/poundage', 'TokenController@poundage');

    //设置币种是否开启提现审核
    Route::post('/poundage/setting', 'TokenController@setting');

    //提现审核通知短信
    Route::get('/phone', 'QclodeController@send');

    //公账信息
    Route::get('/account/information', 'UsdtDetail@account');
