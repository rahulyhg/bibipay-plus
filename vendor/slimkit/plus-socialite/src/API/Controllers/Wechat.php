...
{   
    ...
    public function getUser(Request $request) {
        ...
        $user = $this->httpGet('https://api.weixin.qq.com/sns/userinfo?access_token=' . $access_token . '&openid='. $openid . '&lang=zh_CN`');
    ...
    public function calculateConfig(Request $request) {
       ...
       // return $config;
        if(!$accessToken || !$jssdkTicket) {
            ...
            $result = json_decode($this->httpGet($originUrl), true) ?? [];
            ...
            $jssdkTicketOrigin = json_decode($this->httpGet('https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token='. $accessToken. '&type=jsapi'), true) ?? [];
