...
use Zhiyi\Plus\Models\QzOrder;
use Zhiyi\Plus\Models\QzOkdata;
use Zhiyi\Plus\Models\BaseUser;
use Zhiyi\Plus\Models\QzExerDetail;
...
class QzDetailsController extends Controller
{
    ...
    public function index(Request $request, ResponseContract $response , QzOrder $QzOrder)
    {
        //获取用户信息
      	...
      	$bb = $response->json($token);
      	Log::info('产品列表页面token：'.$bb);
        $user = $request->user();
      	$aa = $response->json($user);
      	Log::info('用户的信息：'.$aa);
        //获取状态值
        ...
        if(isset($status)) {
           ...
           foreach ($pro_order as $value){
                if(time() > $value['pay_end_time']){
                  	//return $response->json($pro_order);
    ...
    public function delOrder(Request $request, ResponseContract $response , QzOrder $QzOrder)
    {
  	...
  	public function exe_detail(Request $request, ResponseContract $response){
    	$user = $request->user();
      	$user_id = $user->id;
      	$base_id = BaseUser::where('sns_uid' , $user_id)->first();
      	$uid = $base_id->id;
    ...
    //用户行权
    public function execute(Request $request, ResponseContract $response , QzOrder $QzOrder, tokenModel $token){
      	$user = $request->user();
        //获取订单信息
        ...
        $exercise_price = $product_detail->exercise_price;
      	//获取钱包信息
        $wallet = $token->where('user_id' , $base_user->id)->where('type_name' , 'USDT')->first();
      	//可兑换ipc价格
      	$token_amount = $order->token_amount;
      	//$exchange_ipc = $request->input('exchange_ipc');
      	$exchange_ipc = $token_amount*1000;
        ...
        if ($exercise_price < $ipc_price){
            ...
            $all_ipc = $pay_money/$this->avgPrice($product_id);
          	//判断用户余额是否充足
          	if($wallet->balance < $pay_money){
            	return $response->json(['message' => '余额不足','code' => '40020']);
            }
          	//处理usdt钱包余额
            $token_data = [
                'balance' => $wallet->balance - $pay_money,
                'total_expenses' => $wallet->total_expenses + $pay_money
            ];
            $token->where('user_id' , $base_user->id)->where('type_name' , 'USDT')->update($token_data);
            //创建账户usdt流水记录
            $log_data = [
            ...
            $usdt_goout = logModel::insertGetId($log_data);
          	//获取ipc钱包信息
        	$ipcwallet = $token->where('user_id' , $base_user->id)->where('type_name' , 'IPC')->first();
          	//处理ipc钱包余额
            $ipcmy_data = [
                'balance' => $ipcwallet->balance + $exchange_ipc,
                'total_expenses' => $ipcwallet->total_income + $exchange_ipc
            ];
            $token->where('user_id' , $base_user->id)->where('type' , 1)->update($ipcmy_data);
          	//创建用户赚取ipc记录
          	$ipc_data = [
              	'tid' =>$id,
            	'user_id' => $base_user->id,
          	...
          	$ipc_income = logModel::insertGetId($ipc_data);
          	//处理行权详情
          	$exet_data = [
              	'uid' =>$base_user->id,
            	'outcome' => $pay_money,
            ...
            $all_ipc = $deposit/$this->avgPrice($product_id);
          	//获取ipc钱包信息
          	$wallet_ipc = $token->where('user_id' , $base_user->id)->where('type_name' , 'IPC')->first();
          	//处理钱包余额
            $token_data = [
                'balance' => $wallet_ipc->balance + $all_ipc,
                'total_expenses' => $wallet_ipc->total_income + $all_ipc
            ];
            $token->where('user_id' , $base_user->id)->where('type_name' , 'IPC')->update($token_data);
          	//用户获取ipc的log记录
          	$ipc_data = [
              	'tid' =>$id,
            	'user_id' => $base_user->id,
    ...
    public function price($id ,$token_num){
        //获取后台提交的产品信息
        $proportion = productModel::where('id' , $id)->first();
      	//获取用户行权价格
    ...
    public function forceExecute(){
        ...
        foreach ($all_order as $order){
            ...
            //查询用户
            $base_user = BaseUser::where('sns_uid' , $order['user_id'])->first();
            $product_detail = productModel::where('id' , $order['product_id'])->first();
            //处理订单状态
            $status = QzOrder::where('id' , $order['id'])->update(['status' => 2]);
          	//获取用户钱包信息
        	$wallet = tokenModel::where('user_id' , $base_user->id)->where('type_name' , 'IPC')->first();
          	//处理钱包余额
            $token_data = [
                'balance' => $wallet->balance + $all_ipc,
                'total_expenses' => $wallet->total_income + $all_ipc
            ];
            tokenModel::where('user_id' , $base_user->id)->where('type_name' , 'IPC')->update($token_data);
            //创建用户赚取ipc记录
            $ipc_data = [
                'tid' =>$order_id,
                'user_id' => $base_user->id,
    ...
    protected function rightTime($id){
        $product = productModel::select('exercise_start_time' , 'exercise_end_time')
            ->where('id' , $id)->first();
