...
use Zhiyi\Plus\Models\BaseUser as BaseUserModel;
...
class CertificationController extends Controller
{
    ...
    public function passCertification(Request $request, Certification $certification)
    {
		...
		$user_id = $certification->user_id;
      	$update = BaseUser::where('sns_uid' , $user_id)->update(['auth' => 1]);
        $certification->user->sendNotifyMessage('user-certification:pass', '你申请的身份认证已被通过', [
            'certification' => $certification,
        ]);
		Log::info('认证的资料详情'.$certification);
    ...
    public function rejectCertification(Request $request, Certification $certification)
    {
        ...
        $certification->data = $data;
      	$certification->reject_content = $content;
        ...
        if ($certification->save()) {
          ...
          BaseUserModel::where('sns_uid', $certification->user_id)->update(['auth' => 2]);
          $user = BaseUserModel::where('sns_uid', $certification->user_id)->first();
            $log = Certification::where('user_id', $certification->user_id)->first();
            $return = (new QclodeController())->revoke($user['name'], $user['tel'], 206394, $log['reject_content']);
            return response()->json(['message' => ['驳回成功'], $return], 201);
    ...
    public function update(
        ...
        return $certification->getConnection()->transaction(function () use ($files, $type, $certification) {
            ...
            $certification->save();
          	BaseUserModel::where('sns_uid', $certification->user_id)->update(['auth' => 1]);
