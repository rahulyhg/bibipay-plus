...
class User extends Authenticatable implements JWTSubject
{
     ...
     */
    protected $hidden = [
        'password', 'remember_token','pivot', 'paypass',
    ];
    ...
    public function createPaypass(string $paypass): self
    {
        $this->paypass = app('hash')->make($paypass);
        return $this;
    }
    public function createRepass(string $repass): self
    {
        $this->repass = app('hash')->make($repass);
        return $this;
    }
    /**
     * 验证用户密码
    ...
    public function verifyPaypass(string $paypass): bool
    {
        return $this->paypass && app('hash')->check($paypass, $this->paypass);
    }
    /**
     * 用户未读数统计.
     *
